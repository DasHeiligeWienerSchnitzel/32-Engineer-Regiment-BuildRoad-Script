	...
	*/
	...
	if (count _nearbyObjects == 0) then {
		...
		*/
		...
		_vehicle setVariable ["ER32_buildRoad_sandHeight",_height,true];
	}else{
		hint "To close!"
	};
}else{
	[_vehicle, 1, ["ACE_SelfActions","ER32_buildRoad_sandDropper_loopActive"]] remoteExecCall ["ace_interact_menu_fnc_removeActionFromObject",0];
	[_vehicle, 1, ["ACE_SelfActions"], ER32_buildRoad_sandDropper_loopDeactive] remoteExecCall ["ace_interact_menu_fnc_addActionToObject",0];
	...
	while {_sandDropperActive == true and _sandAmount > 0} do {
		...
		_height = _vehicle getVariable ["ER32_buildRoad_sandHeight", 0];
		...
		if (_canSpawn == true) then {
			...
			placedSands pushBack [_droppedSand,_posBehindVehicle];
			...
			[_droppedSand] remoteExec ["ER32_fnc_mpScale",0];
			
			_sandAmount = _vehicle getVariable ["ER32_buildRoad_sandFilled",0];
		
			format ["Sand: %1/1500",_sandAmount] remoteExec ["hintSilent",0]  ;
