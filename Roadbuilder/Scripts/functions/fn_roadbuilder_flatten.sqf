...
while {_flattenActive == true} do {
	...
	if (_sandNearby == true) then {
		...
		_sand = createVehicle ["Land_DirtPatch_02_F", _dirtPile select 1, [], 0, "CAN_COLLIDE"];
		_cutter = createVehicle ["Land_ClutterCutter_large_F", _dirtPile select 1, [], 0, "CAN_COLLIDE"];
		
		if (!isNil "ER32_fnc_persistency_saveObject") then {
			[[_sand,_cutter]] remoteExecCall ["ER32_fnc_persistency_saveObject",2];
		};
